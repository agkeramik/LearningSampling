...
double MinimumDistanceBFSqrd(const Path &p1, const Path &p2)
{
    ...
    const unsigned int N2=p2.size();
    for(unsigned int i=0;i<N1;++i){
        unsigned int inext=(i+1)%N1;
        for(unsigned int j=0;j<N2;++j){
            //d=distance from point p1[i] to segment (p2[j],p2[(j+1)%N2])
            double d=DistanceFromSegmentSqrd(p2[j],p2[(j+1)%N2],p1[i]);
            if (d<distance)
                distance=d;
            d=DistanceFromSegmentSqrd(p1[i],p1[inext],p2[j]);
            if (d<distance)
                distance=d;
        }
    }
...
double DistanceFromSegmentSqrd(const IntPoint &seg1, const IntPoint &seg2,const IntPoint &p)
{
    ...
    if(l2==0)
        return DistanceSqrd(p,seg1);
    const double t=((p.X-seg1.X)*(seg2.X-seg1.X)+(p.Y-seg1.Y)*(seg2.Y-seg1.Y))/l2;
    if(t<0.0)
        return DistanceSqrd(p,seg1);
    if(t>1.0)
        return DistanceSqrd(p,seg2);
